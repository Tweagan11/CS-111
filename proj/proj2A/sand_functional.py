...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    """
    try:
        if grid.get(x_from, y_from) == 's' and grid.get(x_to, y_to) is None:
            if x_to - x_from == 0:
                return True
            elif x_to - x_from == 1:
                if grid.get(x_to,y_from) is None or grid.get(x_from, y_to) is None:
                    return True
            elif x_to - x_from == -1:
                if grid.get(x_to, y_from) is None or grid.get(x_from, y_to) is None:
                    return True
...
def do_move(grid, x_from, y_from, x_to, y_to):
    ...
    """
    new_grid = grid.copy()
    if (is_move_ok(new_grid,x_from,y_from,x_to,y_to)):
        new_grid.set(x_to,y_to, new_grid.get(x_from,y_from))
        new_grid.set(x_from, y_from, None)
    return new_grid
...
def do_gravity(grid, x, y):
    ...
    """
    new_grid = grid.copy()
    if new_grid.get(x,y) == 's':
        if is_move_ok(new_grid,x,y,x,y+1):
            new_grid = do_move(new_grid,x,y,x,y+1)
        elif is_move_ok(new_grid,x,y, x-1,y+1):
            new_grid = do_move(new_grid,x,y, x-1, y + 1)
        elif is_move_ok(new_grid,x,y, x+1, y+1):
            new_grid = do_move(new_grid,x,y,x+1, y + 1)
    return new_grid
...
def do_whole_grid(grid):
    ...
    # while not check:
    #    prev = new_grid.copy()
    for y in reversed(range(new_grid.height)):
        for x in range(new_grid.width):
            new_grid = do_gravity(new_grid, x, y)
        # if prev == new_grid:
        #     check = True
    return new_grid
